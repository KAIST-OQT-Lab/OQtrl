...
class adwinSetting():
    ...
    def todict(self)->dict:
        ...
        return {types:asdict(self.__dict__[types]) for types in self.__dict__.keys() if types != 'ASSIGNED'}
    ...
    class _general():
        ...
        PROCESS_DELAY:cond_real = cond_real(1000,minvalue=1,types=int) #Process delay in unit of ns
        EXPERIMENT_MODE:Literal['SINGLE','CONTINUOUS','SERIES'] = OneOf('CONTINUOUS','SINGLE','CONTINUOUS','SERIES')

    @dataclass
    class _DO():
        DO_FIFO_CH_PATTERN:bit_string = bit_string(maxsize=32) #This value will be assigned from the master sequence
        DO_FIFO_WRITE_COUNT:int = cond_real(types=int)                          #This value will be assigned from the master sequence unless user specifies
    ...
    @dataclass
    class _AO():
        AO_UPDATE_PERIOD:cond_real = cond_real(3000,minvalue=3000,types=int) #This value will be assigned from the master sequence
...
class masterSequenceSetting():
    ...
    class general():
        ...
        PROCESS_DELAY:cond_real = cond_real(minvalue=1,types=int) #Process delay in unit of ns
...
class slaveSequenceSetting():
    ...
    def __init__(self,**kwagrs) -> None:    
        ...
        self.__GENERAL = self.general(name=_name, channel=_channel, duration=_duration, update_period=_update_period,types=_types)
...
class settingValidator():
    ...
    def __init__(self, adw:adwinSetting, mas:masterSequenceSetting, slv:slaveSequenceSetting) -> None:
        self.adw = adw
        self.mas = mas
        self.slv = slv
...
class settingAssigner():
    ...
    def _assign_DO_FIFO_WRITE_STARTING_INDEX(self):
        if self.mas.DO.DO_FIFO_WRITE_STARTING_INDEX is not None:
            self.adw.DO.DO_FIFO_WRITE_STARTING_INDEX = self.mas.DO.DO_FIFO_WRITE_STARTING_INDEX
